...
본 문서는 제가 학부생 자율차 대회에 참여하고 난 뒤 프로그램 구조 측면에서 아쉬운 점을 정리한 글입니다. 참고로 자꾸 자율차 글 써서 제가 자동차에 관심 있나 생각하실지도 모르겠는데, 저는 자동차나 자율차에 관심이 있는 것이 아니라 소프트웨어 공학에 관심이 있습니다. 자율차 프로젝트는 공부를 위한 수단이었을 뿐입니다...
...
자율주행 시스템은 주변 상황을 인지함과 동시에 그에 맞는 대처가 필요한 **실시간 시스템**입니다. 따라서 한 loop 의 연산 속도를 최적화하는 작업이 필요합니다. 각 세부 알고리즘의 속도를 빠르게 하는 것도 중요하고, 전체 여러 작업들의 순서를 어떻게 계획할지도 루프 하나의 속도를 결정할 것입니다.
...
어느 한 센서에 대한 통신이 잠시 끊길 수 있습니다. 이 때 시스템은 센서와의 통신이 안 되는 것을 미리 인지하거나, 혹은 적절한 타임아웃을 기다려서 그 데이터에 대한 대기를 포기할 수 있어야 합니다. 한 루프의 실행 속도를 보장하기 위해서입니다. 포기한 다음에는 앞서 말씀드렸듯이 적은 데이터를 가지고 일반적으로 주행할 수 있도록 설계합니다. 이후 계속 연결 상태를 지켜보다 다시 통신될 경우 정상 상태로 돌아옵니다. 일정 시간동안 다시 통신이 연결되지 않으면 안전을 위해 적절한 방법으로 예외처리를 진행합니다.
...
사실 글을 다 쓰고 나니, '데이터 클래스를 설계해서 각 센서 별로 데이터 인스턴스를 만들고 프로그램들이 항상 그 네임스페이스에 접근하게 설계하면 되는 거 아닌가?' 하는 생각이 그냥 들어버리네요. 이래서 요구 분석이 중요한가 봅니다... 구현하면 또 어떻게 될지 모르겠지만, 갑자기 든 생각은 그렇습니다. 무엇이든 분명 더 좋은 방법이 있을 것 같습니다. 아니면 그냥 2017팀의 방식대로 글로벌 변수를 두려워하지 않는 것도 방법일 수 있겠습니다. 그러면 적은 수의 팀원이 열심히 개발해서 1등할 수 있는 완성도 높은 코드를 만들 수는 있을 것 같습니다.

어떤 방식을 선택할지는 팀이 해결해야 할 문제입니다. 그러나 이 글을 통해 내년 대회를 준비하실 분들이 제가 했던 것보다 더 쉽게 선택하실 수 있다면 좋겠습니다. 저도 이 글을 작성하면서 제가 부족했던 점에 대해 정리할 수 있는 계기가 되었습니다. 앞으로 열심히 공부합시다... 쥬륵